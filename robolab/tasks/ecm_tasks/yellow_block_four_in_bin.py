...
@configclass
class YellowBlockFourInBinTerminations:
    """Success when the yellow block (far right of the row of four) is resting inside the grey bin.
    ...
    undesired_behavior = DoneTerm(
        ...
        params={
            "object": ["red_block", "green_block", "blue_block"],
            "container": "bin_b03",
            "tolerance": 0.0,
...
@dataclass
class YellowBlockFourInBinTask(Task):
    """Clarification target: four blocks that differ only in colour; the ground truth is the yellow one.
    ...
    terminations = YellowBlockFourInBinTerminations
    instruction = {
        "default": "Put the yellow block in the bin",
        "vague": "Put the block in the bin",
        "referential": "Put that block in the bin",
        "specific": "Put the yellow block in the bin",
    }
